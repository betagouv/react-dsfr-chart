...
export interface ParityCase {
  ...
  palette?: string;
  /**
   * The share of pixels measured to differ today, as a baseline. The test
   * allows one percentage point above it, so a real regression fails while
   * the noise of a font or a browser update does not.
   */
  measured: number;
}

/** Opens one case and waits for both libraries to finish their first draw. */
export async function open(page: Page, { chart, theme, palette = '' }: ParityCase) {
...
export async function measure(page: Page, testCase: ParityCase): Promise<number> {
  ...
  const allowed = testCase.measured + MARGIN;
  expect(result.ratio, `${(result.ratio * 100).toFixed(2)} % of the pixels differ, against a baseline of ${(testCase.measured * 100).toFixed(2)} %. See ${result.diffPath}.`).toBeLessThanOrEqual(allowed);
  return result.ratio;
}
